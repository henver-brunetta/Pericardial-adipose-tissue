...
data_human <- read_excel("proteome_human_lean_obese_pAT_full_list.xlsx", sheet = 1)
...
for (i in seq_len(nrow(data.clean))) {
  t <- t.test(as.numeric(data.clean[i, 4:10]), as.numeric(data.clean[i, 11:16]), var.equal = TRUE) # test groups
  p.values[i] <- t$p.value
  fc <- mean(as.numeric(data.clean[i, 11:16]))/mean(as.numeric(data.clean[i, 4:10])) # calculate fold change 
  fc.vec <- c(fc.vec, fc)
}
...
pca.data.prot$Groups <- c("lean", "lean", "lean", "lean", "lean", "lean", "lean",
...
colnames(significant_proteins) <- c("CODE", "ID_HUMAN", "ID", 
                            "lean1", "lean2", "lean3", "lean4", "lean5", "lean6", "lean7",
                            "obese1", "obese2", "obese3", "obese4", "obese5", "obese6",
                            "Accession", "fc", "adj.p")

df_heatmap <- significant_proteins %>%
  dplyr::select(-CODE, -ID_HUMAN, -ID, -Accession, -fc, -adj.p, -20, -21)
