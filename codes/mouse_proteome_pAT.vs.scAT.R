...
data_mouse <- read_excel("iWAT vs pWAT mouse analysis full.xlsx", sheet = 1)

data.clean <- na.omit(data_mouse) #clean to 1440
str(data.clean)
...
for (i in seq_len(nrow(data.clean))) {
  t <- t.test(as.numeric(data.clean[i, 4:8]), as.numeric(data.clean[i, 9:13]), var.equal = TRUE) # test groups
  p.values[i] <- t$p.value
  fc <- mean(as.numeric(data.clean[i, 9:13]))/mean(as.numeric(data.clean[i, 4:8])) # calculate fold change 
  fc.vec <- c(fc.vec, fc)
}
...
clean_prot <- clean_stats %>%
  dplyr::select(-CODE, -HUMAN, -ID, -17, -fc, -p.fdr, -fcvolc, -pvolc)
...
pca.data.prot$Groups <- c("iWAT", "iWAT", "iWAT", "iWAT", "iWAT",
                          "pWAT", "pWAT", "pWAT", "pWAT", "pWAT")
...
colnames(significant_proteins) <- c("CODE", "ID_HUMAN", "ID", 
                                    "iWAT1", "iWAT2", "iWAT3", "iWAT4", "iWAT5",
                                    "pWAT1", "pWAT2", "pWAT3", "pWAT4", "pWAT5",
                                    "Accession", "fc", "adj.p")

df_heatmap <- significant_proteins %>%
  dplyr::select(-CODE, -ID_HUMAN, -ID, -Accession, -fc, -adj.p, -17)
...
ggplot(data = vp, aes(x = fcvolc, y = pvolc, col = diffexpressed)) + 
  ...
  coord_cartesian(ylim = c(0, 3), xlim = c(-5, 8)) +
